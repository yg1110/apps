...
-- 전제: `notify-comment` 함수가 먼저 배포되어 있어야 함.
--   supabase functions deploy notify-comment --no-verify-jwt
-- 아래 <PROJECT_REF>를 실제 프로젝트 ref로 교체.
-- (pg_net 확장 필요: Supabase 대시보드 Database → Extensions에서 pg_net 활성화,
--  또는 대시보드 Database → Webhooks에서 GUI로 생성해도 됨.)
-- ============================================================
...
--     url     := 'https://<PROJECT_REF>.functions.supabase.co/notify-comment',
